...
description: 根据当前 branch 与目标 branch 的差异，自动生成 Pull Request 的 Title 与 Description。当用户提到「PR」、「Pull Request」、「写 PR」、「PR 描述」、「PR description」、「创建 PR」时触发此 Skill。
---
...
使用 `references/pr-template.md` 中的模板生成 Description。
...
> **规则：必须为「修改的内容」中列出的每一个模块 / 组件都生成至少一个对应的测试案例，确保所有变更皆被涵盖。**
...
- **`references/pr-template.md`** — PR Description 的完整模板，可按团队需求定制
