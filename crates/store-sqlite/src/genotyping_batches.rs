...
fn batch_from_row(row: &SqliteRow) -> StoreResult<GenotypingBatch> {
    Ok(GenotypingBatch {
        ...
        created_by: optional_uuid(row.try_get("created_by").map_err(map_sqlx)?)?,
        source_attachment_id: optional_uuid(
            row.try_get("source_attachment_id").map_err(map_sqlx)?,
        )?,
...
pub(crate) async fn evidence_for_records(
    ...
    );
    {
        ...
        for record_id in record_ids {
            separated.push_bind(record_id.to_string());
        }
    }
    ...
    for row in &rows {
        ...
        let attachment_id: Option<String> = row.try_get("attachment_id").map_err(map_sqlx)?;
...
async fn locked_batch(
    tx: &mut sqlx::Transaction<'_, Sqlite>,
    id: Uuid,
) -> StoreResult<GenotypingBatch> {
    sqlx::query("UPDATE genotyping_batches SET updated_at = updated_at WHERE id = ?")
        .bind(id.to_string())
        .execute(&mut **tx)
        .await
        .map_err(map_sqlx)?;
    let row = sqlx::query(&format!(
        "SELECT {BATCH_COLUMNS} FROM genotyping_batches WHERE id = ? AND deleted_at IS NULL"
    ))
    .bind(id.to_string())
...
pub(crate) async fn create(
    ...
    .await?;
    require_same_uuid(definition_lab, batch.lab_id, "genotyping batch definition")?;
    if let Some(project_id) = batch.project_id {
        let project_lab = required_lab_id(&mut tx, "projects", project_id, "project").await?;
        require_same_uuid(project_lab, batch.lab_id, "genotyping batch project")?;
    }
    sqlx::query(
    ...
    .bind(batch.lab_id.to_string())
    .bind(batch.project_id.map(|id| id.to_string()))
    .bind(&batch.batch_number)
    .bind(batch.genotype_definition_id.to_string())
    .bind(batch.assessed_at)
    .bind(&batch.method)
    .bind(&batch.notes)
    .bind(encode(&batch.status)?)
    .bind(batch.created_by.map(|id| id.to_string()))
    .bind(batch.source_attachment_id.map(|id| id.to_string()))
...
pub(crate) async fn get(pool: &SqlitePool, id: Uuid) -> StoreResult<GenotypingBatch> {
    let row = sqlx::query(&format!(
        "SELECT {BATCH_COLUMNS} FROM genotyping_batches WHERE id = ? AND deleted_at IS NULL"
    ))
    .bind(id.to_string())
...
pub(crate) async fn list(
    ...
    let status = filter.status.map(|status| encode(&status)).transpose()?;
...
pub(crate) async fn set_preview(
    ...
    let row = sqlx::query(&format!(
        "UPDATE genotyping_batches
         SET source_attachment_id = ?, preview_hash = ?, preview_row_count = ?,
             updated_at = ?, revision = ?
         WHERE id = ? AND revision = ? AND status = 'draft' AND deleted_at IS NULL
         RETURNING {BATCH_COLUMNS}"
    ))
    .bind(preview.source_attachment_id.to_string())
...
async fn ensure_commit_evidence(
    ...
    let project_id = batch.project_id.map(|id| id.to_string());
...
async fn ensure_record_scope(
    ...
    let animal_lab = required_lab_id(tx, "animals", record.animal_id, "animal").await?;
    require_same_uuid(animal_lab, batch.lab_id, "genotyping batch animal")?;
    if let Some(project_id) = batch.project_id {
        let assigned: i64 = sqlx::query_scalar(
            "SELECT EXISTS(
                SELECT 1 FROM project_animal_assignments
                WHERE project_id = ? AND animal_id = ? AND lab_id = ? AND deleted_at IS NULL
             )",
        )
...
async fn insert_batch_record(
    ...
    append_derived_animal_event_tx(tx, &event, audit)
        .await
        .map(|_| ())
}
...
pub(crate) async fn commit(
    ...
    .await?;
    require_same_uuid(definition_lab, before.lab_id, "genotyping batch definition")?;
    if let Some(project_id) = before.project_id {
        let project_lab = required_lab_id(&mut tx, "projects", project_id, "project").await?;
        require_same_uuid(project_lab, before.lab_id, "genotyping batch project")?;
    }
    ...
    let row = sqlx::query(&format!(
        "UPDATE genotyping_batches
         SET status = 'committed', committed_at = ?, updated_at = ?, revision = ?
         WHERE id = ? AND revision = ? AND status = 'draft' AND deleted_at IS NULL
         RETURNING {BATCH_COLUMNS}"
    ))
...
pub(crate) async fn find_for_record(
    ...
    let row = sqlx::query(&format!(
        "SELECT {BATCH_COLUMNS}
         ...
         WHERE gbr.record_id = ? AND gb.deleted_at IS NULL"
    ))
