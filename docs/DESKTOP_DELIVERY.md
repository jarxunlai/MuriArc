...
5. 新安装默认显示已启用的 DeepSeek 预设和“等待配置个人 API”，且不下载模型、不发出外部请求。保存 Provider、模型、上下文/输入/输出/历史、Temperature、超时与视觉设置后重启，确认全部保持并真实用于调用；API Key 只显示“已配置”，不进入数据库、日志、审计、快照或前端状态。切换 Provider 不复用旧 Key，清除 Key 后再次启动仍保持清除状态。
