...
- 基因鉴定批次附件只能由 `ManageBreeding` 角色在 Draft 阶段增删；正式确认要求同一原始
  结果附件完整性校验通过且至少存在一张 `image/*` 胶图。确认后附件锁定，任何校验或事务失败
  都不得产生部分 `GenotypingRecord`。
- AI 的基因鉴定查询只返回动物、正式 `GenotypingRecord`、来源批次以及胶图的受限元数据；
  不向模型暴露附件相对路径、内容 hash、原始结果表附件、创建者账号或对象存储信息。AI 不提供
  创建、预览、提交、取消批次或增删胶图的工具；这些操作必须由有权限的人员在专用 UI 中完成
  预览和确认。
