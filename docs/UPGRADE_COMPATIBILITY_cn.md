...
当前 preview 已实现兼容控制合同，但永久兼容下限和正式发布身份只有在最终 `1.0.0 / E0001` 制品完成完整 RC 后才开始。

## 兼容下限

永久兼容承诺从首个稳定 `1.0 / E0001` 开始。当前 `0.1.0` 登记为 `preview_epoch_0`，用于显式采用、测试并转换为首个不可变 fixture。管理员可以从一个稳定版本直接请求升级到最新稳定版本，控制器内部可跨多个 Epoch。
