...
async function createExport() {
  await createAndDownload(
    () => dataGateway.createExport('xlsx', currentProjectId.value),
    currentProjectId.value ? '项目动物 Registry 导出已生成' : '动物 Registry 导出已生成',
  )
}
...
async function createAndDownload(factory: () => Promise<DataArtifact>, success: string) {
  busy.value = true
  try {
...
<template>
  <div class="page">
    <PageHeader title="数据中心" description="CSV/XLSX 仅导入动物登记或实验测量；结构化导出仅生成动物 Registry 数据产品，完整业务归档使用快照。">
      <template #actions>
        <n-button v-if="exportAllowed" secondary :loading="busy" @click="createExport">
          <template #icon><Download :size="17" /></template>导出动物 Registry
        </n-button>
    ...
    <section class="data-layout" :class="{ compact: !importAllowed }">
      ...
      <aside class="principles surface">
        ...
        <ul>
          ...
          <li><CheckCircle2 :size="15" />普通导入：动物登记、实验测量</li>
          <li><CheckCircle2 :size="15" />普通导出：动物 Registry CSV/XLSX</li>
          <li><CheckCircle2 :size="15" />快照：完整业务归档；当前不可 restore/apply</li>
        </ul>
        <n-button v-if="snapshotAllowed" block secondary :loading="busy" @click="createSnapshot">
...
@media (max-width: 600px) { .steps span { display: none; }.steps li:not(:last-child)::after { right: 4px; left: 28px; }.import-selection { grid-template-columns: 1fr; }.drop-zone { min-height: 220px; text-align: center; }.mapping-table > div { grid-template-columns: 1fr 1fr 70px; padding: 0 7px; }.job-list article { grid-template-columns: 32px 1fr auto; } }
</style>
